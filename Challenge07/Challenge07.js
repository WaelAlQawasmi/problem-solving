...
const applicationsStatics = (arr) => {
    for (let index = 0; index < arr.length; index++) {
            ...
            if(arr[index].LastName==null&&arr[index].firstName==null||arr[index].yearsOfExperience<1){
                result.rejectedApplicants++;
              }
              switch(arr[index].tech){
                case Java:result.java_Devs++; 
                break;
                case python:result.python_Devs++; 
                break;
                case javaScript:result.javaScript_Devs++; 
                break;
                case dotNet:result.dotNet_Devs++; 
                break;
              }
